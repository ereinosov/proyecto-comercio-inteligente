...
- `clasificar_merma` — dos ramas (research.md #5, #10):
  ...
  - **con `id_conteo_renglon`** (BLOQUEADA por 001 User Story 5, T065-T071): clasifica la
    `diferencia` que 001 expone. Hoy devuelve `409 caja_bloqueado_por_001` — comportamiento
    controlado y esperado (tasks.md T023).
...
from rasero.persistencia.modelos import Existencia, Lote, Merma, Producto, Sucursal
...
def clasificar_merma(
    ...
    *,
    ...
    if id_conteo_renglon is not None:
        # BLOQUEADA por 001 (User Story 5, T065-T071). El esquema de la FK existe; la lógica de
        # servicio que lee `conteo_renglon` no. El 409 es esperado y controlado (tasks.md T023).
