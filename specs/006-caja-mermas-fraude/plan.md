...
| VI. Autorización y Roles (enmienda v2.3.0) | **Cumple** | La única acción de este módulo sujeta a rol —anular una venta de un turno cerrado— verifica rol `encargado` o superior por el mecanismo central `requiere_rol` (implementado en la User Story 10 de `001`), no por una lectura directa de `es_encargado`. `006` no gestiona operadores ni sucursales; consume `id_operador` y su rol, nunca el PIN. Esta fila se añade al sincronizar la enmienda v2.3.0. |
