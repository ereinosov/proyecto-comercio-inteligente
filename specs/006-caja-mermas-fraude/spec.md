...
1. **Campos y tablas de `001` que ya existen para esto** (verificado contra
   `001/data-model.md`, constitución v2.4.0): `existencia` (agregación derivada, sin restricción de
...
**Propiedad de datos — estado frente a la constitución (v2.4.0)**: la tabla de Propiedad de Datos
...
- **Datos personales**: `006` no captura ningún dato de cliente. Solo consulta el `id_operador` y su
  rol; nunca el PIN ni el hash del PIN. Los rastros de este módulo no contienen
  datos de pago completos (constitución, Principio IV). La verificación de rol para anular una
  venta de turno cerrado pasa por el mecanismo central `requiere_rol` (Principio VI, enmienda
  v2.3.0), no por una lectura directa de `es_encargado`.
- **Constitución vigente citada**: v2.4.0. Este módulo **no motivó ninguna enmienda** (v2.2.6,
  sobre `007`; v2.3.0, Principio VI sobre el esquema de `operador` de `001`; y v2.4.0,
  sub-sección "Identidad de sesión" del Principio VI, son posteriores y sin efecto estructural
  sobre este módulo más allá de usar `requiere_rol`): sus tres
