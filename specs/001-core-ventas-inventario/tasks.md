...
### User Story 12 — Catálogo de productos con imagen (Fase 3b de la ronda de evolución visual)

- **Migración `0011_producto_url_imagen`**: `producto.url_imagen TEXT NULL`, aditiva, sin migración de datos. `alembic upgrade`/`downgrade` reversible verificado.
- **Backend**: `catalogo.listar_catalogo` y `GET /productos` devuelven `url_imagen`; `ProductoCuerpo` y el alta/edición de Producto de Administración lo aceptan; `_valida_url_imagen` exige `http(s)` bien formada o vacía (`admin_url_imagen_invalida`). Tests nuevos en `tests/integracion/test_administracion.py` (persistencia + rechazo de URL malformada + vacío → `null`).
- **Frontend**: componente `ImagenProducto` (`<img>` con `onError` → ícono de familia de categoría, reutilizando `familiaDeCategoria`/`IconoCategoria`); catálogo en grid + filtro de chips por categoría en Venta; layout de Venta de una a **dos columnas** (catálogo · ticket) con encabezado a lo ancho y total/Cobrar anclados. Se apila en vertical bajo el breakpoint. `SelectorProducto` sigue disponible sin cambios de comportamiento. El "+" de cada tarjeta NO usa Verde Rasero (La Regla de la Sola Voz).
- **Test de interfaz — excepción puntual al Principio III**: se añadió `vitest` + `@testing-library/react` sólo para cubrir el fallback de `ImagenProducto` (`ImagenProducto.test.tsx`), por pedido explícito de esta fase. No abre la puerta a pruebas de maquetación; es un test de comportamiento de un componente con lógica de estado (imagen ↔ ícono).
- **DESIGN.md**: enmienda MENOR sobre v1.5.0 — imágenes de producto por URL externa con fallback obligatorio (riesgo de disponibilidad aceptado) y layout de dos columnas de Venta.

### Corrección 2026-09-07 — bloqueo duro de venta/traspaso sin existencia suficiente

Reversión de FR-047 (la cita del Principio II estaba mal aplicada). No reabre el conteo
histórico "146/146"; son tareas nuevas.

- [X] T147 Nueva excepción `ExistenciaInsuficiente` (`codigo` `existencia_insuficiente`, 409) en `backend/rasero/errores.py`; el handler global de `ErrorDominio` ya la cubre — sin registro por endpoint. Confirmado que 409 no colisiona con los códigos 409 existentes.
- [X] T148 `backend/rasero/persistencia/movimientos.py`: `obtener_existencia_total(sesion, *, id_sucursal, id_producto)` — suma de todos los lotes del producto en la sucursal.
- [X] T149 `backend/rasero/servicios/ventas.py`: `registrar_venta` valida cada renglón (sumado por producto) contra `obtener_existencia_total` ANTES de escribir ningún movimiento; rechaza la venta COMPLETA con `ExistenciaInsuficiente` si un renglón no alcanza. `_consumir_fefo` deja de imputar el exceso a saldo negativo (raise defensivo). `_cantidad_base_renglon` centraliza la validación de forma del renglón.
- [X] T150 `backend/rasero/servicios/traspasos.py`: `despachar_traspaso` aplica el mismo bloqueo contra la existencia de la sucursal de origen, validando todos los renglones antes de crear el traspaso.
