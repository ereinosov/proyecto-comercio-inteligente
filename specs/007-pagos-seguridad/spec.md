...
Las cinco entidades que la constitución (v2.4.0) asigna a `007-pagos-seguridad` en su tabla de
...
- **Reservas al encargado**: cambiar el catálogo o la cobertura de medios de pago, registrar o mover
  ...
  cada servicio), y todas quedan en la bitácora. **Enmienda v2.4.0 ("Identidad de sesión")**: la
  identidad del operador en esas acciones se deriva ahora del token de sesión de turno (header
  `Authorization: Bearer`), no del `id_operador` del cuerpo, que se retira del schema. La
  verificación de rol sigue siendo `requiere_rol`; sólo cambia de dónde sale la identidad.
- **Constitución vigente citada**: v2.4.0. Este módulo **motivó la enmienda v2.2.6** (2026-09-05),
